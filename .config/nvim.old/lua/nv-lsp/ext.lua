...
if expand_snippet then
  if item.textEdit then
    vim.api.nvim_call_function("UltiSnips#Anon", {item.textEdit.newText .. suffix})
  else
    vim.api.nvim_call_function("UltiSnips#Anon", {item.insertText .. suffix})
  end
end
end
...
function M.attach(client, bufnr)
  ...
  "autocmd InsertCharPre <buffer=%d> lua require('nv-lsp.ext')._InsertCharPre(%s)",
  bufnr,
  client.config.flags.server_side_fuzzy_completion
  ))
  vim.cmd(string.format("autocmd InsertLeave <buffer=%d> lua require('nv-lsp.ext')._InsertLeave()", bufnr))
  vim.cmd(string.format("autocmd CompleteDone <buffer=%d> lua require('nv-lsp.ext')._CompleteDone()", bufnr))
  if (client.server_capabilities.completionProvider or {}).resolveProvider then
    vim.cmd(string.format("autocmd CompleteChanged <buffer=%d> lua require('nv-lsp.ext')._CompleteChanged()", bufnr))
  end
